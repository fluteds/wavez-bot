...
const MAX_CONTENT = 300; // anything longer is rejected outright with 400 BAD_REQUEST; !help at 311 chars was the first to hit it
const MAX_CHUNKS = 4; // a runaway reply should not carpet the room

// Long replies go out as several messages, split on a space where there is one, hard cut where a single word is oversized.
...
export class WavezBot {
  constructor({ baseURL, botToken, roomId, config = {} }) {
    ...
    this.roomId = roomId;
    this.config = config;
    // locale only reaches SDK-side messages; server errors are pt-BR until Accept-Language says otherwise.
