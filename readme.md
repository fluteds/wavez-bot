...
- Chat messages are capped server-side at 300 characters, and a longer one is rejected outright with a bare `400 BAD_REQUEST` rather than a truncation. `bot.reply` splits anything longer on a space, up to four messages, so `!help` and the other list commands survive the room growing. The exact ceiling was not probed past 300; if a reply still bounces, lower `MAX_CONTENT` in [lib/bot.js](lib/bot.js).
